 //------------------------------------------------------------------------------------//
  // TILE LAYER FOR ATTRIBUTION --------------------------------------------------------//
  //------------------------------------------------------------------------------------//
  var baseMap = 'https://cartodb-basemaps-{s}.global.ssl.fastly.net/dark_all/{z}/{x}/{y}.png';
  ...
  var layer = new L.TileLayer(colorMap, {
...
//------------------------------------------------------------------------------------//
  ...
  var animateFlightsBtn = L.easyButton({
    states: [{
            stateName: 'startAnimate', // STATE NAME
            icon:      '<img src="img/play.png">',
            title:     'Show live flight data', 
            onClick: function(btn, map) {   
                animatePaths()
                btn.state('stopAnimate'); // CHANGE STATE ON CLICK
            }
        }, {
            stateName: 'stopAnimate',
            icon:      '<img src="img/stop.png">',
            title:     'Stop live flight data',
            onClick: function(btn, map) {
                stopAnimation()
                btn.state('startAnimate');
            }
    }]
});
map.fitBounds(ne2);

animateFlightsBtn.addTo(map);

  //------------------------------------------------------------------------------------//
  // FLIGHTS ---------------------------------------------------------------------------//
  //------------------------------------------------------------------------------------//
  /*
  15.347286, -158.456221 - lo us
  45.746833, -49.561230 - hi us
  https://opensky-network.org/api/states/all?lamin=15.347286&lomin=-158.456221&lamax=45.746833&lomax=-49.561230
  */
   /* NE ONLY
  https: //opensky-network.org/api/states/all?lamin=42.075743&lomin=-79.742226&lamax=47.261665&lomax=-67.515351
  */
  /* CENTER VERMONT 
  42.968564, -73.166415
  */
...
function getBounds(a) {
    // map.getBounds()
    lamin = a._southWest.lat;
    lomin = a._southWest.lng;
    lamax = a._northEast.lat;
    lomax = a._northEast.lng;
    return 'https://opensky-network.org/api/states/all?lamin='+lamin+'&lomin='+lomin+'&lamax='+lamax+'&lomax='+lomax+'';
}

map.on('moveend', function() { 
   getFlights(getBounds(map.getBounds()), listFlights)
});

map.on('load', getFlights(getBounds(map.getBounds()), listFlights))

  var newengland2 = 'https://opensky-network.org/api/states/all?lamin=39.560217&lomin=-80.740297&lamax=47.578931&lomax=-66.062839';
  var usa = 'https://opensky-network.org/api/states/all?lamin=15.347286&lomin=-158.456221&lamax=45.746833&lomax=-49.561230';
  var newengland= 'https://opensky-network.org/api/states/all?lamin=42.075743&lomin=-79.742226&lamax=47.261665&lomax=-67.515351';
  
  function getFlights(a, cb) {
  ...
  //------------------------------------------------------------------------------------//
  // UNIX EPOCH TIME CONVERT -----------------------------------------------------------//
  //------------------------------------------------------------------------------------//
  
  function convertTime(a) {
      var utcSeconds = a;
      var d = new Date(0) || 1234567890;
  ...
  var plane0 = L.icon({
      iconUrl: 'img/flight_icon_teal.png',
      iconSize:     [15, 15],
      iconAnchor:   [10, 10],
      popupAnchor: [0, -10],
  ...
  var airportMarker = L.icon({
    iconUrl: 'img/airportmarker.png',
    iconSize:     [15, 15],
    iconAnchor:   [10, 10],
    popupAnchor: [0, -10],
    className: 'planeClass' 
  ...
  function listFlights(a) {
    flights = JSON.parse(a)
  
    for(var i = 0; i < flights.states.length; i++) {
        
        // ADD SINGLE MARKER
        if(planes[i] != undefined) {
        map.removeLayer(planes[i])
        }
    
        planes[i] = L.marker([flights.states[i][6], flights.states[i][5]], {
        title: "Flight: "+ flights.states[i][1] +" ", icon: plane0, rotationAngle: flights.states[i][10]
        }).addTo(map);
        
        // BUILD POPUP DETAILS FOR EACH PLANE
        planes[i].bindPopup('<h2>Flight: ' + flights.states[i][1] + flights.states[i][2]
            + '</h2><p>Current status: ' + getStatus(flights.states[i][8]) + '</p>', {
        closeOnclick: false
        })
  ...
  airports = []
  function addAirports(a) {
    airports = a;
    console.log(airports)
    for(var j = 0; j < airports.length; j++) {
        usaAirports[j] = L.marker([airports[j].lat, airports[j].lon], {
            title: airports[j].name, state: airports[j].state, city: airports[j].city, icon: airportMarker
        })
        airports[j].bindPopup('<h2>' + airports[j].options.title + '</h2><p>City: ' + airports[j].options.city+ '</p>' + '<p>State: ' + airports[j].options.state+ '</p>', {
        closeOnclick: false
        })
        airports[j].addTo(map)
    }
}

addAirports(usaAirports)